...
import {Inbox} from 'app/client/contacts/inbox.model';
...
export class ContactDetailComponent implements OnInit, OnDestroy {
        ...
        this._activatedRoute.params.subscribe(params => {
            if (params['ID2']) {
                ...
                this._contactsService.getContactById(this.contactId).then(res => {
                    ...
                    if(this.inbox.status=='unread'){
                        this._contactsService.update(
                            { id: this.inbox.id, status: "read",type:"status" }
                        )
                    }
                }).catch(err => {
                });
    ...
    toCrm(note): void {
        this.isLoading = true;        
        this._contactsService.toCRM({id:this.inbox.id,note:note,type:"spreadsheet"}).then(res => {
            this.isLoading = false;
            if(res.status){
                this.inbox.ex_to_sheet =1;
            ...
            this._matSnackBar.open('Error saving to CRM!', 'OK', {
                verticalPosition: 'top',
                duration: 3500
            });
            this.isLoading = false;
    ...
    openCRMNoteDialog(){
        const dialogRef = this.dialog.open(CrmDialogComponent, {
            width: '600px',
            data: {note: ""}
          });
