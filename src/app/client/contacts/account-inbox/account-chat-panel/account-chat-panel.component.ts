...
@Component({
    selector     : 'account-chat-panel',
    templateUrl  : './account-chat-panel.component.html',
    styleUrls    : ['./account-chat-panel.component.scss'],
    encapsulation: ViewEncapsulation.None
})
export class AccountChatPanelComponent implements OnInit, AfterViewInit, OnDestroy
{
    contacts: any[];
    chat: any;
    selectedContact: any;
    ...
    )
    {
        ...
        this._accountInboxService.onContactIsClickFromInboxList.subscribe(contact=>{
    ...
    toggleChat(contact): void
    {
        ...
        else
        {
            ...
            this._accountInboxService.contactIsClickFromChatPanel(this.in_account_id,this.selectedContact.ID1,contact).then(res => {
    ...
    reply(event): void
    {
        ...
        this._accountInboxService.sendInboxMessage(data_message).then(response => {
            this._prepareChatForReplies();
        });
    }
}
